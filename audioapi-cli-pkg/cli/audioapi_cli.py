...
@click.option('--api-endpoint', type=str, help="Use an alternative API endpoint (without the 'wss://' prefix)", default=api.AudioAPI.get_default_api_endpoint())
...
@click.option('--dst-path', type=str, help=f'Local path to download the enhanced file [default: <local_path>/<original_filename>_enhanced.wav]')
...
@click.option('--api-token', type=str, help='Authentication key to access the AudioAPI service', prompt='API token', required=True)
@click.option('--api-endpoint', type=str, help="Use an alternative API endpoint (without the 'http://' prefix)", default=api.AudioAPI.get_default_api_endpoint())
...
@click.option('--src-path', type=str, help='If choose <src-type=local> then <src-path> should point the full path of the source file on the local machine. \
                                            If choose <src-type=remote> then <src-path> should contain the URL of the source file', \
                                            prompt='Source path', required=True)
@click.option('--no-download', is_flag=True, help="If set, the enhanced file won't be downloaded to the local machine (we'll get only the URL of the enhanced file)")
@click.option('--dst-path', type=str, help=f'Local path to download the enhanced file [default: <local_path>/<original_filename>_enhanced.wav]')
@click.option('--retention', type=str, help='URL Retention duration [minutes]')
@click.option('--status-interval', type=int, help='Check the audio enhancement process every <status-interval> [seconds]', default=AudioEnhancer.get_default_status_interval())
